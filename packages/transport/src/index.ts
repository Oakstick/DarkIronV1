...
function decodeSceneLoaded(event: SceneEvent): SceneLoadedEvent {
  ...
  for (let i = 0; i < scene.meshesLength(); i++) {
    ...
    // Decode MaterialData if present
    const matFb = mesh.material(new MaterialData());
    const material: MaterialInfo | null = matFb
      ? {
          name: matFb.name(),
          baseColorPath: matFb.baseColorPath(),
          normalPath: matFb.normalPath(),
          roughnessPath: matFb.roughnessPath(),
          metallicPath: matFb.metallicPath(),
        }
      : null;
